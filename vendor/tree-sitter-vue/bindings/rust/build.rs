fn main() {
    ...
    // NOT upstream. `src/` is generated by tree-sitter-cli and vendored verbatim,
    // and it does not compile warning-free: scanner.c has `\`-continued `//`
    // comments (-Wcomment), a wint_t/char comparison (-Wsign-compare) and two
    // intentional switch fall-throughs (-Wimplicit-fallthrough). Cargo hides
    // build-script output for registry dependencies, which is why the other
    // ~30 tree-sitter grammars are quiet and this one -- a path dependency --
    // is not. Since we do not maintain this source, silence `cc`'s -Wall/-Wextra
    // rather than patch generated code we would have to re-patch on every
    // regeneration.
    c_config
        .warnings(false)
