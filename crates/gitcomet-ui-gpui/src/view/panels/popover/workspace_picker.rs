...
pub(super) fn suggested_worktree_path(repo: &RepoState, query: &str) -> String {
    ...
    match repo.spec.workdir.parent() {
        // Forward slashes keep the suggestion platform-stable (and git accepts
        // them everywhere); `Path::display` would render the platform
        // separator, which the tests and the dialog's consumers need not know.
        Some(parent) => parent.join(folder).to_string_lossy().replace('\\', "/"),
        None => folder,
    }
}
